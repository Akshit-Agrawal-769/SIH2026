  ...
  return (
    <div className="instrument-well p-3 flex flex-col gap-2 font-mono text-xs select-none">
      ...
      <div className="flex items-center justify-between border-b border-[var(--border-subtle)] pb-1.5">
        <div className="flex items-center gap-1.5 text-sky-400">
          ...
          <span className="font-bold uppercase tracking-wider text-[11px] text-slate-100">
            ROMS 4D Numerical Model
          </span>
        </div>
        <span className="text-[10px] text-sky-300 font-sans">
          Spatio-temporal Colocated
        </span>
      </div>
      ...
      <div className="bg-[var(--surface-base)] border border-[var(--border-hairline)] rounded-[2px] p-2 flex items-center justify-center relative overflow-hidden">
        {validPairs.length > 0 ? (
          <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full h-auto">
            ...
            <text x={svgWidth - padRight} y={padTop - 8} textAnchor="end" fill="#38bdf8" fontSize="9" fontFamily="monospace">
            ...
            {validPairs.map((p) => {
              ...
              return (
                ...
                >
                  <title>{`Depth: ${p.depth}m | Model: ${p.val.toFixed(2)} ${unit}`}</title>
                </circle>
              );
            })}
          </svg>
          ...
          <div className="py-24 text-center text-slate-500 text-[11px]">
            No model values available for this profile.
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between text-[10px] text-slate-400 px-0.5">
        <span>Discrete Levels: <strong className="text-slate-200">{validPairs.length}</strong></span>
        <span>Surface Value: <strong className="text-sky-300">{validPairs[0]?.val?.toFixed(2) ?? '—'} {unit}</strong></span>
      </div>
    </div>
  );
};
